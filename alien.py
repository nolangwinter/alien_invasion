...
class Alien(Sprite): 
    alien_images = [[pg.transform.rotozoom(pg.image.load(f'images/alien_3{n}.png'), 0, 2.5) for n in range(2)],
                    [pg.transform.rotozoom(pg.image.load(f'images/alien_1{n}.png'), 0, 2.5) for n in range(2)],
                    [pg.transform.rotozoom(pg.image.load(f'images/alien_2{n}.png'), 0, 2.5) for n in range(2)]]
    ...
    def __init__(self, game, type, alien_number):
        ...
        self.timer_explosion = Timer(Alien.alien_explosion_images[type], delay=500, is_loop=False)
    ...
    def hit(self):
        if not self.dying:
            ...
            self.sb.increment_score(Alien.alien_points[self.type])
    def update(self): 
...
class Extra(Sprite):
    extra_image = [pg.transform.rotozoom(pg.image.load('images/UFO_0.png'), 0, 5)]
    aelist100 = [100, 100, 'blank']
    aelist200 = [200, 200, 'blank']
    aelist300 = [300, 300, 'blank']

    extra_points = [300, 200, 100]

    extra_explosion_images = [[pg.transform.rotozoom(pg.image.load(f'images/explosion_{el}.png'), 0, 2.5) for el in aelist300],
                                [pg.transform.rotozoom(pg.image.load(f'images/explosion_{el}.png'), 0, 2.5) for el in aelist200] ,
                                [pg.transform.rotozoom(pg.image.load(f'images/explosion_{el}.png'), 0, 2.5) for el in aelist100]]
    ...
    def __init__(self, game):
        ...
        self.image = pg.transform.rotozoom(pg.image.load(f'images/UFO_0.png'), 0, 4)
        self.timer_normal = Timer(image_list=Extra.extra_image)
        self.timer_explosion = Timer(self.extra_explosion_images[self.rand], delay=500, is_loop=False)  
        self.timer = self.timer_normal    
        self.dying = self.dead = False
    ...
    def hit(self):
        if not self.dying:
            self.dying = True
            self.timer = self.timer_explosion
            self.scoreboard.increment_score(Extra.extra_points[self.rand])


    def check_collision(self):
        collisions = pg.sprite.groupcollide(self.game.extra, self.ship_lasers, False, True)
        if collisions:
            self.hit()
